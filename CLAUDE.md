...
- 同事位置地圖：稽查員看「所屬公司」同事；主管看「管轄公司」；點位最後回報超過 2 小時顯示灰色，**超過 4 小時不顯示於地圖**（名單仍保留並標「地圖未顯示」）。

## 回報類型（4 種）
| 類型 | 圖示 | 顏色 | 需填地點+內容 |
|------|------|------|:---:|
| 到達 | 📍 | 綠 | ✅ |
| 離開 | 🚗 | 橘 | ⬜（地點/內容為**選填**，欄位仍顯示，可填可空）|
| 隨車(上車) | 🚌 | 藍 | ✅ |
| 隨車(下車) | 🚏 | 紫 | ✅ |
...
- 我的紀錄 → 兩個 Tab：紀錄查詢（支援日期區間 startDate/endDate）/ 日報表
...
- 分組管理
- 狀態板（今日已回報/未回報）
- 日報表：與稽查員前台同款正式樣板（見下方）。篩選含日期/公司/組別/**人員**
  - 「人員」選「全部」＝多人/全體版；選**單一人員**＝代印該員的「稽查員工作日報表」（單人版，主管於稽查員不在時代印用）
