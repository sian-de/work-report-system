...
## 稽查員日報表（report.html「我的紀錄 → 日報表」）
正式化版面（「稽查員工作日報表」）：
- **正式抬頭**：公司名稱（登入回傳 `companyName`）＋報表標題＋製表人/期間/製表日期/編號
- **摘要**（列印亦顯示）：總筆數 / 回報天數 / 類型數 / 平均每日筆數 ＋ 各類型統計長條
- **明細表格**：`# | 日期時間（MM/DD＋時間換行）| 類型 | 地點 | 處理內容 | GPS`
  - 欄寬：# 5% / 日期時間 11% / 類型 10% / 地點 18% / 處理內容 44% / GPS 12%
  - 列內日期僅 MM/DD（完整日期在分組標題與抬頭期間）；GPS 經緯度保留（螢幕＋列印）
- **簽核欄**：製表人 / 單位主管 / 核閱（三格）
- **頁尾**：製表系統＋製表時間（頁碼用 `@page` margin box，Chrome 不支援則略過）
- 列印：A4 直式，含抬頭→摘要→明細→簽核完整一份
